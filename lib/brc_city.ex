defmodule BrcCity do
  ...
  def dispatch_line(state, line) do
    ...
    Agent.cast(pid, BrcCity, :update_worker, [temperature])
    new_state
  end
  ...
  @spec update_worker({integer(), integer(), integer(), integer()}, integer()) ::
  ...
  defp gather_lines(<<"\n", rest::binary>>, acc, dispatch_pid) do
    # do things with acc
    Agent.cast(dispatch_pid, BrcCity, :dispatch_line, [acc])
